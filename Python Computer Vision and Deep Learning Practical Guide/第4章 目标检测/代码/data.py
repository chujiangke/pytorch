...
class DetectionData(Dataset):
    def __init__(self, folder=target_folder, subset="train", transform=None):
        ...
        annotation_paths = [
            re.sub(".jpg", ".txt", path) for path in image_paths
        ]
        image_paths_train, image_paths_test, annotation_paths_train, annotation_paths_test = train_test_split(
            image_paths, annotation_paths, test_size=0.2, random_state=20
        )
        if subset == "train":
